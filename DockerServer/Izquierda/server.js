...
mongoose.connect('mongodb://dnscont02.southcentralus.azurecontainer.io:27018/izquierda', (err) => {
    if (err) throw err;
...
http.listen(process.env.PORT || 3004, () => {
	console.log("Listening on port %s", process.env.PORT || 3004);
});
...
var timer_id;
  ...
  app.post('/izquierda', function (req, res) {
    publish("get/izquierda", "IZQUIERDA", options);
...
app.post('/bizquierda', function (req, res) {
  publish("get/izquierda", "IZQUIERDA", options);

  var movi = new ChatModel({_id:uuid.v1(),mov:"izquierda",RegisterDate:moment().format("YYYY-MM-DD hh:mm:ss"),Duration:"00:00:01",place:"brazo mioelectrico"});
  ...
  res.send('movimiento izquierda guardado');
});
