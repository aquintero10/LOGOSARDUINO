...
mongoose.connect('mongodb://dnscont02.southcentralus.azurecontainer.io:27019/prenderLED', (err) => {
    if (err) throw err;
...
var timer_id;
  ...
  app.post('/prenderLed', function (req, res) {
    publish("get/prenderled", "LED", options);
...
/************************brazo*************************************************/
app.post('/bprenderLed', function (req, res) {
  publish("get/prenderled", "LED", options);

  var movi = new ChatModel({_id:uuid.v1(),mov:"prender Led",RegisterDate:moment().format("YYYY-MM-DD hh:mm:ss"),Duration:"00:00:01",place:"brazo mioelectrico"});
  ...
  res.send('movimiento prender led guardado');
});
